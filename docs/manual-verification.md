...
3. Confirm the generated wrapper at `~/.fluent-frame/bin/native-host` includes absolute `FF_YTDLP_PATH` and `FF_CODEX_PATH` exports. If either tool was not found, rerun the installer with `FF_YTDLP_PATH=/absolute/path/to/yt-dlp` and `FF_CODEX_PATH=/absolute/path/to/codex`.
...
- Repeat the wrapper edit with `export FF_CODEX_PATH=/missing/codex` and confirm the extension reports that Codex is missing, then restore the wrapper.
