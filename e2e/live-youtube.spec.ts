...
test("generates subtitles on a real YouTube watch page through the real native host", async () => {
  ...
  try {
    ...
    await expect(page.locator("#ff-status")).toContainText(/Learning subtitles (streaming|ready)/, { timeout: 300_000 });
    await expect(page.locator("#ff-current-phrase")).not.toHaveText("");
    await page.locator("#ff-current-phrase [data-action='current-jump']").first().click();
