...
function prepareCaptionBatches(captionText: string): string[] {
  ...
  if (cues.length <= AGENT_CUE_BATCH_SIZE) {
    return [captionText];
  }

  const batches: string[] = [];
  for (let index = 0; index < cues.length; index += AGENT_CUE_BATCH_SIZE) {
    const batch = cues.slice(index, index + AGENT_CUE_BATCH_SIZE);
    const batchNumber = Math.floor(index / AGENT_CUE_BATCH_SIZE) + 1;
    const batchCount = Math.ceil(cues.length / AGENT_CUE_BATCH_SIZE);
    const formattedBatch = batch.map(formatCue).join("\n\n");
    batches.push(
      `NOTE: The source caption file is long. Process this ${AGENT_CUE_BATCH_SIZE}-cue batch ${batchNumber} of ${batchCount} only. Preserve these cue IDs and timings exactly.\n\n${formattedBatch}\n`,
    );
  }
  return batches;
}
